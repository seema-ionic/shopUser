...
/* harmony import */ var ng_circle_progress__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ng-circle-progress */ "./node_modules/ng-circle-progress/__ivy_ngcc__/fesm2015/ng-circle-progress.js");
...
Tab1PageModule = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_2__["NgModule"])({
        imports: [
            ...
            _tab1_routing_module__WEBPACK_IMPORTED_MODULE_6__["Tab1PageRoutingModule"], _pipes_pipes_module__WEBPACK_IMPORTED_MODULE_9__["PipesModule"],
...
let Tab1Page = class Tab1Page {
    constructor(apiservices, router, zone, menuCtrl, sendData, storage, storageService, eventService, platform, statusBar) {
        ...
        this.TrendingNews = {
            ...
            speed: "500", initialSlide: 0, slidesPerView: 1, spaceBetween: 10, pager: true,
        };
        this.ToDoListSlider = { initialSlide: 0, spaceBetween: 5, slidesPerView: 1, zoom: false };
        this.errorMessage = "";
        this.value = 0;
    ...
    ionViewWillEnter() {
        ...
        this.storage.get('fcmToken').then((fcmToken) => {
            ...
            if (this.newFcmTokenKey == undefined || this.newFcmTokenKey == '') {
    ...
    redirectionDetail(trendingList) {
        this.zone.run(() => {
            this.sendData.myParam = trendingList;
    ...
    homeAllApi() {
        this.zone.run(() => {
            ...
            let apikey = {};
            this.apiservices.apiCallWithLoginToken(_src_assets_constant__WEBPACK_IMPORTED_MODULE_4__["URLS"].homeAllData, apikey).subscribe((result) => {
                console.log("home page all data api result==", result);
                if (result.status == 1 || result.status == '1') {
                    this.allData = result.data;
                    this.sayCheersData = this.allData.badgelist;
                    this.quickAccessArray1 = this.allData.quickaccess.list;
                    this.quickAccessArray = [];
                    for (let i = 0; i < this.quickAccessArray1.length; i++) {
                        if (i < 3) {
                            this.quickAccessArray.push(this.quickAccessArray1[i]);
                            console.log("menu data1==", this.quickAccessArray);
                        }
                        console.log("menu data==", this.allData.quickaccess.list);
                    }
                    console.log("say cheers data==", this.allData);
                    console.log("say cheers data==", this.sayCheersData);
                    console.log("this.allData.quickaccess.list==", this.allData.quickaccess.list);
                    this.value = 0;
    ...
    initialFun() {
        this.zone.run(() => {
            this.storageService.getStorage().then(storedData => {
                ...
                this.apiservices.apiCallWithLoginToken(_src_assets_constant__WEBPACK_IMPORTED_MODULE_4__["URLS"].initialApi, apiKey).subscribe((result) => {
    ...
    gotoAwardDetail(item) {
        this.sendData.myParam = item;
        this.router.navigate(['/award-detail']);
        // this.router.navigate(['/award-dashboard']);
    }
    awardListFun() {
        this.zone.run(() => {
            ...
            this.storageService.getStorage().then(stored => {
                ...
                console.log("resend OTP by Voice Call, api key", apiKey);
                this.apiservices.apiCallWithLoginToken(_src_assets_constant__WEBPACK_IMPORTED_MODULE_4__["URLS"].aboardListApi, apiKey).subscribe((result) => {
                    console.log("resend OTP by Voice Call, api response==", result);
                    this.apiservices.hideLoader();
                    this.awdDetailData = result;
                    if (result.status == 1 || result.status == '1') {
                        this.welcomeAboardData = result.data;
                        this.errorMessage = "";
                        // this.apiservices.showToastMessage(result.message, 'top', 3000, 'greenBg');
                    }
                    else {
    ...
    quickAccessRedirection(quickAccessList) {
        this.zone.run(() => {
            if (quickAccessList.link != '') {
                if (quickAccessList.identifier == "external") {
                    this.apiservices.iab.create(quickAccessList.link, "_system", this.apiservices.options);
                }
                else {
                    this.sendData.alldata = quickAccessList;
                    let data = {
                        "commingFrom": "homePage"
                    };
                    this.sendData.commingPageName = data;
                    console.log("community quickAccessList==", quickAccessList);
                    this.router.navigate([quickAccessList.link]);
                }
            }
            else { }
        });
    }
    communityPostRedirection(allData, scrollValue) {
        this.zone.run(() => {
            console.log("community data==", allData);
            this.sendData.myParam = allData;
            this.sendData.autoScroll = scrollValue;
            this.router.navigate(["/community-detail"]);
        });
    }
    gotoWelcomeAboard(val, item) {
    ...
    communityList() {
        this.zone.run(() => {
            ...
            this.storageService.getStorage().then(storedData => {
                ...
                this.apiservices.apiCallWithLoginToken(_src_assets_constant__WEBPACK_IMPORTED_MODULE_4__["URLS"].homePostApi, apikey).subscribe((result) => {
                    ...
                    if (result.status == 1 || result.status == '1') {
                        ...
                        if (this.postData == undefined) {
                            this.postData = result.data.list;
                            console.log("noticeBoard List Infinite Scroll if1==", this.postData);
                        }
                        else {
                            this.postData = this.postData.concat(result.data.list);
                            console.log("noticeBoard List Infinite Scroll else1==", this.postData);
                        }
                        if (this.myinfiniteScrollCall == true) {
                            console.log("noticeBoard List Infinite Scroll if2==", this.postData);
                            this.myInfiniteScroll.target.complete();
                            this.errorMessage = "";
                            this.myinfiniteScrollCall = false;
                        }
                        console.log("thoughtOftheDayApi List data1==", this.postData);
                        console.log("thoughtOftheDayApi List data2==", this.errorMessage);
                    }
                    else {
                        this.errorMessage = result.message;
                        console.log("noticeBoard List Infinite Scroll12==", this.postData);
                        if (this.myinfiniteScrollCall == true) {
                            console.log("noticeBoard List Infinite Scroll123==", this.postData);
                            this.myInfiniteScroll.target.complete();
                            this.myinfiniteScrollCall = false;
                        }
                    }
    ...
    doInfinite(infiniteScroll) {
        ...
        setTimeout(() => {
            this.value = this.postData.length;
    ...
    sidemenu() {
        this.zone.run(() => {
            ...
            let apikey = {};
            this.apiservices.apiCallWithLoginToken(_src_assets_constant__WEBPACK_IMPORTED_MODULE_4__["URLS"].sideMenu, apikey).subscribe((result) => {
                console.log("side menu api result==", result);
                if (result.status == 1 || result.status == '1') {
                    this.apiservices.innerHeaderImage = result.data.settings;
                    this.apiservices.sideMenuList = result.data.menuData;
                    this.headerImage = result.data.settings;
                }
                else {
                    this.apiservices.showToastMessage(result.message, 'top', 3000, 'redBg');
                }
    ...
    goToSayCheers(value, index) {
        this.zone.run(() => {
            if (value == "cheerList") {
                ...
                let obj = {
                    allData: this.sayCheersData.list,
                    index: index,
                    pageHeader: this.sayCheersData.settings,
                };
                this.sendData.sayCheers = obj;
                // this.sendData.myParam="cheerboard";
                // this.sendData.myParam=this.sayCheersData.settings.requestTab;
                this.router.navigate(["/say-cheers-slide-detail"]);
            }
            else {
                this.router.navigate(["/say-cheers-board"]);
            }
        });
    }
    goToProfile() {
    ...
    gotoWelcomeAboardList() {
        this.router.navigate(["/welcome-aboard-list"]);
    }
    gotoBirthdayList(item) {
        this.sendData.alldata = item;
